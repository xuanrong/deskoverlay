// 锁屏星空场景：椭圆轨道 + 闪烁星点 + 中央太阳辉光的 canvas 绘制。
// 两处消费方共用同一实现，避免视觉分叉（曾各自维护一份完全相同的副本）：
//   1) lock.html —— 系统级置顶锁屏窗口（lockpage.js）
//   2) 主窗口浏览器开发态回退浮层（lock.js）
// 性能：行星渐变仅 resize 时重建、dpr 上限 1.5、星点用 fillRect 替代 arc、
// 星点保持固定位置只做透明度呼吸（不重排）。
...
/// 在 canvas 上启动星空动画，返回停止函数（取消 rAF 与 resize 监听）。
export function startSkyAnim(canvas) {
